...
const TreasuryConsideration = (param: { proposalCost: bigDecimal }) => {
  ...
  function draftTypeRendered() {
    switch (state.draftType) {
      ...
      case "threshold":
        return (
          <ThresholdDraftComponent setConsumer={(mem) => setMember(state.draftType, mem)}></ThresholdDraftComponent>
        );
      case "transfer":
      ...
      {step == 0 && (
        <Form onSubmit={onFormSubmit} className="proposal-form">
          ...
          <Form.Group className="mb-3" controlId="formTaxType">
            ...
            <Form.Text className="text-muted">What treasury action would you like to take?</Form.Text>
          </Form.Group>
