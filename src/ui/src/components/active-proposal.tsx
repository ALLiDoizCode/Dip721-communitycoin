          ...
          <Row className="tabs">
            <Col>
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to right, green " + votingPercents.yay + "%, red " + votingPercents.yay + "%)",
                }}
              >
                <Row className="text-percent">
                  <Col>
                    <span>{votingPercents.yay}%</span>
                  </Col>
                  <Col>VS</Col>
                  <Col>
                    <span>{votingPercents.nay}%</span>
                  </Col>
                </Row>
                <Row className="text-percent">
                  <Col>
                    <span>{bigIntToDecimal(activeProposal.yay).getPrettyValue(3, ",")} YC</span>
                  </Col>
                  <Col>VS</Col>
                  <Col>
                    <span>{bigIntToDecimal(activeProposal.nay).getPrettyValue(3, ",")} YC</span>
                  </Col>
                </Row>
              </div>
            </Col>
            ...
            <Col>
              <Card className="active-proposals">
                ...
                <Card.Body>
                  ...
                  <Card.Text>{activeProposal.description}</Card.Text>
                </Card.Body>
                ...
                <Card.Body>
                  <Row className="vote-buttons">
                    <Col>
                      <button
                        disabled={!connected}
                        onClick={() => showModal("yay")}
                        className="btn btn-success btn-lg vote-button"
                      >
                        Yay
                      </button>
                    </Col>
                    <Col>
                      <button
                        disabled={!connected}
                        onClick={() => showModal("nay")}
                        className="btn btn-danger btn-lg  vote-button"
                      >
                        Nay
                      </button>
                    </Col>
                ...
                <Card.Footer className="text-muted">
                  <div>
                    <span className="card-label">Creator: </span>
                    {activeProposal.creator}{" "}
                  </div>
                  <div>
                    <span className="card-label">Created At: </span>
                    {new Date(Number(activeProposal.timeStamp / 1000000)).toLocaleDateString()}
                  </div>
                </Card.Footer>
              </Card>
            </Col>
      ...
      <Modal show={votingModal} onHide={() => setVotingModal(false)}>
        ...
        <Modal.Body>
          ...
          <div className="input-group mb-3">
            ...
            <input
              min="0"
              onChange={(event) => setVotingPower(new bigDecimal(event?.target?.value || 0))}
              data-placement="top"
              title="Every YC is one vote"
              type="number"
