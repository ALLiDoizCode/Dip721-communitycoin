...
const storageEffect =
  ...
  ({ setSelf, onSet }) => {
    ...
    onSet((newValue, _, isReset) => {
      isReset ? storageType.removeItem(key) : storageType.setItem(key, JSON.stringify({ savedVal: newValue }));
    });
  };
...
export const connectedAtom = atom({
  key: "connected",
  default: false,
  effects: [storageEffect("connected", sessionStorage)],
});

export const identityProviderAtom = atom<string>({
  key: "identityProvider",
  default: undefined,
  effects: [storageEffect("identityProvider", sessionStorage)],
});

export const principalAtom = atom<string>({
  key: "principal",
  default: Principal.anonymous().toText(),
  effects: [storageEffect("principal", sessionStorage)],
});
